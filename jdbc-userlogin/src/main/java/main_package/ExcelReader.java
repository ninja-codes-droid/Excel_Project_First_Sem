	            ...
	            for (int i = row.getRowNum() + 1; i <= sheet.getLastRowNum(); i++) {
	                ...
	                if (nextRow != null) {
	                    ...
	                    data[rowIndex] = new String[] {
	                        paper_code.trim(), paper_title.trim(), coll.trim(), cate.trim(), number.trim(), paper_type.trim(),
	                        full_marks.trim(), marks_obt.trim(), stream.trim(), sem.trim(), year.trim(), subject.trim()
	                    };
	                    rowIndex++;
